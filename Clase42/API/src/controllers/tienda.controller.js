...
const getTienda = async (req, res) => {
  const userInfo = await usersService.getById(req.user?.id || "anonimo");
  res.render("tienda.ejs", userInfo);
};

const getCarrito = async (req, res) => {
  const userInfo = await usersService.getById(req.user?.id || "anonimo");
  res.render("carrito.ejs", userInfo);
};
...
const getDetalle = async (req, res) => {
  const userInfo = await usersService.getById(req.user?.id || "anonimo");
  res.render("detalle.ejs", userInfo);
};
